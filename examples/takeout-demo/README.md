...
The script creates an isolated temporary index, imports the fixture, finds it by its synthetic caption, and writes a JPEG preview into a new private subdirectory of `./stillport-demo-output`. It prints each real JSON result and deletes the temporary index when it exits. The source fixture is never changed. On macOS, it also requires the generated JPEG to match `brand/demo-preview.jpg` byte for byte, so the product page cannot silently drift from the demonstrated artifact.
